-- Migration: Remove approved_by and approved_at columns from timecard_headers table
-- This migration should be run AFTER migrating the data to audit log entries
...
BEGIN
    ...
    IF unmigrated_count > 0 THEN
        RAISE EXCEPTION 'Cannot proceed: % approved timecards have not been migrated to audit log. Run migrate-approval-data-to-audit-log.sql first.', unmigrated_count;
    END IF;
...
DO $$
BEGIN
    RAISE NOTICE 'Successfully removed approved_by and approved_at columns from timecard_headers table';
END $$;